...
class Level1View(arcade.View):
    ...
    def on_update(self, delta_time):
        ...
        old_x = self.sheriff.center_x
        old_y = self.sheriff.center_y
        ...
        if arcade.check_for_collision_with_list(self.sheriff, self.walls_list):
            self.sheriff.center_x = old_x
            self.sheriff.center_y = old_y
    ...
    def on_key_press(self, key, modifiers):
        ...
        # Взаимодействие с дверью (работает только при нажатии E)
        if key == arcade.key.E:
